...
## 六、第二次真实调用：改进有效，但仍不完美

在相同 `CASE-001` 输入下，第二次成功调用得到以下结果：
...
| JSON 约定检查 | 通过 | 通过 |
| 风险状态 | 误报，需要复核 | 通过，无误报 |
| 耗时 | 21.721 秒 | 37.518 秒 |
| 输入 Token | 当时未记录 | 602 |
| 输出 Token | 当时未记录 | 3,617 |
| 总 Token | 当时未记录 | 4,219 |
| 本地预估费用 | 当时未记录 | ¥0.003014 |
...
Prompt 1.2 已通过 50 项自动化测试和 20 条案例静态校验，但尚未进行新的付费模型对比。因此只能说明规则已经实现，不能提前声称真实模型的事实一致性已经提高。
...
1. 使用同一案例真实验证 Prompt 1.2，检查“多设备切换、篇数和具体体验”是否消失；
2. 再决定是否运行全部 20 条付费评测；
3. 对人工评分结果做 Prompt 版本对比；
4. 将本案例制作成两分钟演示视频；
5. 开始第二个 RAG 知识库项目，而不是继续堆叠无实际需求的中间件。

## 十一、简历表述参考

> 开发基于 FastAPI 与千问的电商运营 AI 工作流，将确定性利润计算、结构化策略生成、SQLite 历史回放和用户反馈整合为完整应用；针对真实模型测试中发现的事实编造与风险误报，设计 Prompt 事实白名单和语境化风险检查，并记录 Token、预估费用与调用耗时；构建 20 条离线案例、50 项 pytest 测试和 GitHub Actions 自动回归。

这段表述只描述已经实现并验证的工程能力，不声称提升销量或实现商业化。

## 十二、面试讲解版本

> 我最初完成模型结构化输出后，以为格式通过就代表结果可用。但第一次真实测试发现，模型虽然返回了正确 JSON，却自行编造赠品、试用人数、广告成本和产品能力；风险规则还把“禁止绝对化表述”误判成违规。我随后把风险检查改成语境判断，强化 Prompt 的事实边界，并解析供应商 usage 记录 Token 和费用。第二次测试消除了风险误报和多数明显编造，但仍出现“蓝牙双模等于多设备切换”等推断。因此我没有宣称问题已解决，而是继续把 Prompt 升级为事实白名单，将温度从 0.7 降到 0.2，并用自动化测试锁定规则。这个过程让我理解了格式校验、事实一致性、评测和成本控制是四个不同的问题。
